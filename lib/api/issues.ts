...
export const IssueSingleGetAPI = async (id: string): Promise<Issues | null> =>
  (await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/issues/${id}`, { method: "GET", cache: "no-store" })).json();
